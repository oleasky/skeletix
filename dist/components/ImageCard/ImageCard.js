...
export const ImageCard = ({ image, altText, title, content, quality = 85, children, className = '' }) => {
    return (React.createElement(Container, { htmlTag: "article", className: `md:flex-row bg-[#f1f5f9] ${compId} ${className}`, layout: 'row' },
        image && (React.createElement("figure", { className: 'flex-0 relative md:w-3/5' },
            React.createElement(Image, { src: typeof image === 'string' ? image : image.src, alt: altText, width: 800, height: 800, quality: quality }))),
