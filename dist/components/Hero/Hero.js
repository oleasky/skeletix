...
const compId = 'hero';
export const Hero = ({ className, title, content, imageUrl, imageAlt, quality = 95, children }) => {
        ...
        React.createElement(Container, { layout: 'row', width: 'wide', alignItems: 'items-center', className: `h-[clamp(26rem, 80vw, 50rem)] mx-auto w-full ${compId}__wrapper` },
            imageUrl &&
                React.createElement("figure", { className: `absolute w-[100vw] h-full flex-1 z-0 top-0 left-1/2 -translate-x-1/2 ${compId}__image` },
                    React.createElement(Image, { src: imageUrl, alt: imageAlt || 'Image Alt', fill: true, priority: true, quality: quality, sizes: "(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 100vw", placeholder: 'blur', className: 'object-cover' })),
